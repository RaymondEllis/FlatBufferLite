...
{
_w.AppendLine();
_w.Append("namespace ").AppendLine(ns);
_w.AppendLine("{");
}
for (int i = 0; i < enumCount; i++)
...
{
_w.AppendLine();
_w.Append("[StructLayout(LayoutKind.Explicit, Size = ").Append(s.Size).AppendLine(")]");
_w.AppendLine("public partial struct " + s.Name);
_w.OpenBlock();
foreach (var f in s.Fields)
{
string cs = ResolveTypeName(f.Type);
_w.Append("[FieldOffset(").Append(f.Offset).AppendLine(")]");
_w.Append("public ").Append(cs).Append(' ').Append(ToPascalCase(f.Name)).AppendLine(";");
}
_w.CloseBlock();
}
...
_w.Append("public const int SlotCount = ").Append(t.SlotCount).AppendLine(";");
_w.Append("public const int InlineSize = ").Append(t.InlineSize).AppendLine(";");
_w.Append("public const int InlineAlign = ").Append(t.InlineAlign).AppendLine(";");
