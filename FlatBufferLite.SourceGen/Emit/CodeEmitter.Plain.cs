...
if (typeName == null)
continue;
_w.Append("public ").Append(typeName).Append(' ').Append(ToPascalCase(field.Name)).AppendLine(";");
}
...
_w.Append("FlatBufferCollections.EnsureCreate<").Append(type).AppendLine(">();");
foreach (var type in listTypes)
_w.Append("FlatBufferPlainVectors.EnsureCreate<").Append(type).AppendLine(">();");
_w.CloseBlock();
}
...
{
_w.Append("value.").Append(fieldName).Append(" = ").Append(fieldName).AppendLine(";");
return;
}
...
{
_w.Append("var ").Append(local).Append(" = ").Append(fieldName).AppendLine(";");
_w.Append("if (").Append(local).AppendLine(".IsValid)");
_w.OpenBlock();
_w.Append("var __item = value.").Append(fieldName).AppendLine(".GetValueOrDefault();");
_w.Append(local).AppendLine(".ToPlain(ref __item);");
_w.Append("value.").Append(fieldName).AppendLine(" = __item;");
_w.CloseBlock();
_w.AppendLine("else");
_w.OpenBlock();
_w.Append("value.").Append(fieldName).AppendLine(" = null;");
_w.CloseBlock();
return;
}
if (field.Type.IsVector)
...
_w.Append("if (").Append(source).AppendLine(".IsValid)");
_w.OpenBlock();
_w.Append("var ").Append(target).Append(" = value.").Append(fieldName).AppendLine(";");
_w.Append("if (").Append(target).AppendLine(" == null)");
_w.OpenBlock();
...
{
_w.Append("value.").Append(fieldName).Append(" = ").Append(local).AppendLine(".AsSpan.ToArray();");
}
}
