...
public class GeneratedSchemaTests
{
	...
	{
		Span<byte> buf = stackalloc byte[PlayerRef.GetMaxSize(nameByteCount: 0, inventoryCount: 0)];
		var b = new FlatBufferBuilder(buf);
		PlayerRef.Create(ref b);
	...
	public void PlayerTable_RoundTripsValues()
	{
		...
		Span<byte> buf = stackalloc byte[PlayerRef.GetMaxSize(nameByteCount: 5, inventoryCount: 3)];
		var b = new FlatBufferBuilder(buf);
